...
- 무기 전환 시 비활성 무기 Actor를 유지하지 않고도 HUD, 장비 상태, 전투 흐름이 일관되게 이어지는 경험
- 특수 사격 Mod가 현재 활성 무기의 사격 상태를 바꾸되, 장비 시스템이 개별 Mod 효과 구현을 직접 알지 않아도 되는 경험
- owner-only 정보와 공개 전투 표현을 분리해 멀티플레이 복제 비용을 제어하는 경험

무기 장비 시스템은 더 이상 `장비 데이터를 저장하는 시스템`, `무기 Actor를 여러 개 유지하는 시스템`, `개별 무기별 입력을 직접 분기하는 시스템`이 아니다. 이 문서 기준의 무기 장비 시스템은 **Inventory 기반 소유권, WeaponManager 기반 활성 슬롯 관리, Item 기반 어빌리티 생명주기, ASC 기반 슬롯 자원 정본**을 연결하는 허브다.
...
| `UPRWeaponManagerComponent` | `PrimarySource`, `SecondarySource`, `ActiveSlot` 관리, 활성 무기 전환, 로컬 Actor 갱신 | Character 소유 |
...
| `WeaponActor` | 현재 활성 무기의 로컬 비주얼, 머즐 플래시, 메시, 애니메이션 연동, 표현 전용 상태 | 로컬 스폰 전용 |
...
| 현재 보여 주는 무기 Actor는 무엇인가 | `WeaponManager.LocalActiveActor` |
...
- 현재 활성 무기 1개만 로컬 스폰한다.
- 비활성 무기 Actor는 유지하지 않는다.
- 머즐 플래시, 메시, 소켓, 애니메이션 연동, 공개 표현만 담당한다.
- 손에서 내려놓으면 사라져도 되는 상태만 가진다.
- 게임플레이 상태의 정본이 아니다.
...
```mermaid
flowchart TD
    A[인벤토리 UI에서 무기 장착 요청] --> B[Client가 ServerRPC_EquipWeapon 호출]
    B --> C[Server가 Item 유효성 검증]
    C --> D[PrimarySource 또는 SecondarySource 갱신]
    D --> E[정책상 활성화 대상이면 ActiveSlot 갱신]
    E --> F[대상 Item의 OnEquipped 호출]
    F --> G[모든 클라이언트가 OnRep_ActiveSlot 수신]
    G --> H[기존 LocalActiveActor 정리]
    H --> I[새 무기 Actor를 로컬 스폰 및 부착]
```
...
```mermaid
flowchart TD
    A[무기 전환 입력] --> B[Client가 ServerRPC_SwapActiveWeapon 호출]
    B --> C[Server가 현재 활성 Source의 OnUnequipped 호출]
    C --> D[새 활성 슬롯 결정]
    D --> E[새 Source 기준으로 ActiveSlot 갱신]
    E --> F[새 Source의 OnEquipped 호출]
    F --> G[OnRep_ActiveSlot 기준으로 각 머신이 LocalActiveActor 교체]
...
- 비활성 슬롯은 Actor를 유지하지 않고, 슬롯 데이터와 HUD로만 존재한다.
...
flowchart TD
    ...
    D --> E[현재 활성 무기면 ActiveSlot.ModData 갱신]
...
| `WeaponActor` | 로컬 전용, 각 머신이 자체 스폰 |
| 순간 이펙트 | 예측 + Multicast |

- 공개 상태에는 asset reference만 넣는다.
...
| 활성 무기 Actor 정책 | 로컬 Actor 1개만 유지 |
