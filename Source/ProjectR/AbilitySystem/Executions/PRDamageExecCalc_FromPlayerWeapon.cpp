...
// 정적 싱글톤. ExecutionCalculation 인스턴스가 매번 생성/파괴되더라도 캡처 정의는 한 번만 만들어 재사용
static const FFromPlayerCaptureDefs& GetCaptureDefs()
{
...
{
	// RelevantAttributesToCapture에 등록된 어트리뷰트만 ExecCalc 내부에서 캡처 평가할 수 있다.
	// GE Spec 빌드 시점에 GAS가 이 목록을 보고 Source/Target 어트리뷰트의 스냅샷 또는 라이브 캡처를 준비한다.
	const FFromPlayerCaptureDefs& Defs = GetCaptureDefs();
